...
//       formState: { errors, isSubmitting }, // 에러 및 제출 중 상태
//       reset, // 폼 초기화 함수
//     } = useForm<SignupFormTest>({ mode: 'onChange' }) // 값이 변경될때마다 유효성 검사

//     // 패스워드 표시/감춤 토글상태

//     const [showPassword1test, { toggle, togglePassword1test }] = useToggleState(false)
//     // 첫 번째 비밀번호 입력창의 토글, 토글유틸 포함
//     const [showPassword2test, { toggle, togglePassword2test }] = useToggleState(false)
//     // 두 번째 비밀번호 입력창의 토글, 토글유틸 포함

//     // 폼 제출 시, 실행되는 비동기 함수(실제 가입 로직)
//     const onSubmitTest = async (formData: SignupFormTest) => {

//       if (isSubmitting) return // 폼 제출 중에는 실행되지 않도록 설정

//       // supabase 회원가입 api호출
//       const { error, data } = await supabase.auth.signUp({
//         // 필수 신용 정보 (이메일, 패스워드)
//         email: formData.email, /// 필수 자격 정보 - 이메일
//         password: formData.password, // 필수 자격 정보 - 비밀번호
//         // 선택적 메타데이터 추가(사용자 이름 및 소개 등..)
//         options: {
//           data: { // 추가로 저장할 사용자 메타데이터
//             username: formData.name,  // 사용자 이름
//             bio: formData.bio // 자기소개
//           }
//         }
//       })

//       // supabase 회원가입 api 호출 에러 처리
//       if (error) {
//         // - toast로 오류 상태 알림
//         toast.error(
//           `회원가입 인증 오류 발생! [${error.status} : ${error.name} : ${error.message}]`
...
//         // 회원가입 api 호출 성공 처리
//         if (data.user) {
//           // profiles 테이블에 사용자 정보 저장 (오류 발생 시, toast 알림) / 실제로 유저 객체가 존재(생상)하면 profiles 테이블에 추가 정보 저장
//           const { error } = await supabase.from('profiles').insert({
//             id: data.user.id, // 프로필 PK로 auth 유저 id 사용
//             username: data.user.user_metadata.username, // 메타데이터의 사용자 명
//             email: data.user.user_metadata.email, // 메타데이터의 이메일
//             bio: data.user.user_metadata.bio, // 메타데이터의 자기소개
//             created_at: new Date().toISOString(), // 생성시각(ISO 문자열)
//           })

//           if (error) {
//             // 프로필 저장 실패 시 토스트로 알림
//             toast.error(`프로필 테이블 추가 오류 발생! [${error.name} : ${error.message}]`)
//           } else {
//             // toast로 회원가입 성공 메세지 알림 / 모든 단계 성공 > 성공 토스트 알림 + 페이지 이동 + 폼 초기화
//             toast.success('회원가입에 성공했습니다') // 성공 메세지 출력
//             // 로그인 페이지로 이동(페이지 변경)
//             navigate('signin')
//             // 폼 초기화
//             reset()
...
//     // password2 유효성 검증을 위해 현재 password 값 실시간 참조
//     const password = watch('password')
...
//             <input
//               id="signup-password"
//               type={showPassword1test ? 'text' : 'password'}
//               autoComplete="off"
//               aria-invalid={!!errors.password}
//               aria-describedby={
//                 errors.password ? 'signup-password-error' : undefined
...
//             <button
//               type="button"
//               aria-label={showPassword1test ? '패스워드 감춤' : '패스워드 표시'}
//               title={showPassword1test ? '패스워드 감춤' : '패스워드 표시'}
//               aria-pressed={showPassword1test}
//               className="cursor-pointer absolute right-2 top-2 px-2 py-1 text-sm text-gray-600 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none focus:ring focus:ring-blue-300"
//               onClick={togglePassword1test}
//             >
//               {showPassword1test ? <Eye size={16} /> : <EyeOff size={16} />}
//             </button>
//           </div>
//           {errors.password && (
//             <div
...
//             <input
//               id="signup-password2"
//               type={showPassword2test ? 'text' : 'password'}
//               autoComplete="off"
//               aria-invalid={!!errors.password2}
//               aria-describedby={
//                 errors.password2 ? 'signup-password2-error' : undefined
...
//             <button
//               type="button"
//               aria-label={showPassword2test ? '패스워드 감춤' : '패스워드 표시'}
//               title={showPassword2test ? '패스워드 감춤' : '패스워드 표시'}
//               aria-pressed={showPassword2test}
//               className="cursor-pointer absolute right-2 top-2 px-2 py-1 text-sm text-gray-600 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none focus:ring focus:ring-blue-300"
//               onClick={togglePassword2test}
//             >
//               {showPassword2test ? <Eye size={16} /> : <EyeOff size={16} />}
//             </button>
//           </div>
//           {errors.password2 && (
//             <div
